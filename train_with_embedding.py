...
from model import Classifier, BiLSTM, SelfAttentiveEncoder, AverageMeter
...
f.close()
f = open('/scratch/near/anlp/pos_dictionary.json')
line = f.readlines()
pos_dictionary = json.loads(line[0])
f.close()
...
config['pos_size'] = len(pos_dictionary.items())
config['ninp'] = word2vec_dim + config['pos_size']
config['nhid']= 300
config['nlayers']=2
...
config['pos_matrix'] = torch.eye(config['pos_size'])
config['pos_matrix'][0,0]=0
config['pos_matrix'][1,1]=0
config['pooling'] = 'all'
...
init_range = 0.1
model.encoder.bilstm.pos_embedding.weight.data.uniform_(-init_range, init_range)
model.encoder.bilstm.word_embedding.weight.data.uniform_(-init_range, init_range)
model.encoder.bilstm.pos_embedding.weight[0,:]=0.0
model.encoder.bilstm.pos_embedding.weight[1,:]=0.0
...
for epoch in range(0,50):
    ...
    if valid_loss<min_loss:
        count = 0
        min_loss = valid_loss
        model_path = '/scratch/near/anlp/saved_model_pos_embedding/2/epoch_%d_%.2fmodel'%(epoch,valid_loss)
